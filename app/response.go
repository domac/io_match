...
func acceptVersion(req *http.Request) int {
	if req.Header.Get("accept") == "application/vnd.iomatch; version=1.0" {
		return 1
	}
...
func RespondDefault(w http.ResponseWriter, code int, data interface{}) {
	...
	w.Header().Set("X-IOMATCH-Content-Type", "io-match; version=1.0")
	w.WriteHeader(code)
	w.Write(response)
}
...
func LogPanicHandler(l log.Logger) func(w http.ResponseWriter, req *http.Request, p interface{}) {
...
func LogNotFoundHandler(l log.Logger) http.Handler {
...
func LogMethodNotAllowedHandler(l log.Logger) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		Decorate(func(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
